...
def _calculate_correlation_unit(df_ev_generation: pd.DataFrame, df_charger_generation: pd.DataFrame):
...
def _calculate_dtw_unit(seq1: np.ndarray, seq2: np.ndarray) -> float:
    n = len(seq1)
    m = len(seq2)
...
def calculate_pure_dtw_cost(df_chargers: pd.DataFrame, df_evs: pd.DataFrame) -> pd.DataFrame:
    ...
    def compute_one(i, j, c_vals, e_vals):
        d_ij = _calculate_dtw_unit(c_vals, e_vals)
        return i, j, d_ij

    tasks = []
    for i, c_col in enumerate(charger_cols):
        for j, e_col in enumerate(ev_cols):
            tasks.append((i, j, charger_data[c_col], ev_data[e_col]))

    try:
        results = Parallel(n_jobs=-1, backend="loky")(
            delayed(compute_one)(i, j, c_vals, e_vals) for (i, j, c_vals, e_vals) in tasks
        )
    except Exception:
        try:
            ...
            results = [compute_one(i, j, c_vals, e_vals) for (i, j, c_vals, e_vals) in tasks]

    for i, j, d_ij in results:
...
def calculate_pure_euclidean_cost(df_chargers: pd.DataFrame, df_evs: pd.DataFrame) -> pd.DataFrame:
    charger_cols = df_chargers.columns
    ev_cols = df_evs.columns
    ...
    def compute_one(i, j, c_vals, e_vals, c_ts_local, e_ts_local):
        df_c = pd.DataFrame({"charger_current": c_vals}, index=c_ts_local)
        df_e = pd.DataFrame({"ev_current": e_vals}, index=e_ts_local)

        _, paired = _calculate_correlation_unit(df_e, df_c)
        if len(paired) == 0:
            d_ij = np.inf
        else:
            diff = (
                paired["charger_current"].to_numpy(dtype=np.float32)
                - paired["ev_current"].to_numpy(dtype=np.float32)
            )
    ...
    try:
        results = Parallel(n_jobs=-1, backend="loky")(
            delayed(compute_one)(i, j, c_vals, e_vals, c_ts_local, e_ts_local)
            for (i, j, c_vals, e_vals, c_ts_local, e_ts_local) in tasks
        )
    except Exception:
        try:
            results = Parallel(n_jobs=-1, backend="threading")(
            ...
            results = [
                compute_one(i, j, c_vals, e_vals, c_ts_local, e_ts_local)
                for (i, j, c_vals, e_vals, c_ts_local, e_ts_local) in tasks
            ]
...
def perform_matching_from_cost(cost_matrix_df: pd.DataFrame):
