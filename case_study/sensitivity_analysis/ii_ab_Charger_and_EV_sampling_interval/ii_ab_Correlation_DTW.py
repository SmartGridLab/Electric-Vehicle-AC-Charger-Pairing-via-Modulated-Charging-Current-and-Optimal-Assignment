...
# This metric is robust to time shifts via Dynamic Time Warping (DTW)
...
# Target directory for Correlation-DTW sensitivity analysis
BASE = cwd / "case_study" / "sensitivity_analysis" / "ii_ab_Charger_and_EV_sampling_interval"
ACC_DIR = BASE / "accuracy" / "Correlation-DTW"
...
def run_one_combo(
    ...
    Calculates Pairing accuracy using the Correlation-DTW composite metric.
    """
    num_p = len(command_patterns)
    fname = f"{num_p}p_Chint{charger_interval}_EVint{ev_interval}_Correlation-DTW_Acc.csv"
    ...
    print(f"[RUN] Correlation-DTW: EV={ev_interval}s, CH={charger_interval}s")
